...
var _ = Describe("EndToEnd Smart BFT configuration test", func() {
	...
	BeforeEach(func() {
		...
		for _, peer := range network.Peers {
			...
			address := fmt.Sprintf("127.0.0.1:%d", network.ReservePort())
			core.Peer.Address = address
			core.Peer.ListenAddress = address
			core.Peer.Gossip.Bootstrap = address
			core.Peer.Gossip.Endpoint = address
			core.Peer.Gossip.ExternalEndpoint = address
	...
	Describe("smartbft network", func() {
		It("smartbft multiple nodes stop start all nodes", func() {
			...
			network.UpdateChannelAnchors(network.Orderers[0], channel)
			network.VerifyMembership(network.PeersWithChannel(channel), channel)

			By("enabling V2_0 application capabilities on the channel")
			nwo.EnableCapabilities(network, channel, "Application", "V2_0", network.Orderers[0], peer, network.Peer("Org1", "peer1"), network.Peer("Org2", "peer0"), network.Peer("Org2", "peer1"))
		...
		It("smartbft assisted synchronization no rotation", func() {
			...
			By("updating the anchor peers")
			network.UpdateChannelAnchors(orderer, channel)
			network.VerifyMembership(network.PeersWithChannel(channel), channel)

			By("enabling V2_0 application capabilities on the channel")
			nwo.EnableCapabilities(network, channel, "Application", "V2_0", orderer, peer, network.Peer("Org1", "peer1"), network.Peer("Org2", "peer0"), network.Peer("Org2", "peer1"))

			// Disable leader rotation in application channel
			updateSmartBFTMetadata(network, peer, orderer, channel, func(md *smartbft.ConfigMetadata) {
			...
			Eventually(proc.Ready(), network.EventuallyTimeout).Should(BeClosed())
			Eventually(runner.Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 0, sequence 13"))
			...
			assertBlockReception(map[string]int{"testchannel1": 17}, network.Orderers, peer, network)

			invokeQuery(network, peer, orderer, channel, 0)
			assertBlockReception(map[string]int{"testchannel1": 18}, network.Orderers, peer, network)

			By("Ensuring follower participates in consensus")
			Eventually(runner.Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Deciding on seq 18"))
		})
		...
		It("smartbft autonomous synchronization", func() {
			...
			Eventually(ordererRunners[0].Err(), network.EventuallyTimeout*2, time.Second).Should(gbytes.Say("Changing to follower role, current view: 2, current leader: 3 channel=testchannel1"))
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout*2, time.Second).Should(gbytes.Say("Changing to leader role, current view: 2, current leader: 3 channel=testchannel1"))
			Eventually(ordererRunners[3].Err(), network.EventuallyTimeout*2, time.Second).Should(gbytes.Say("Changing to follower role, current view: 2, current leader: 3 channel=testchannel1"))
			...
			runner.Command.Env = append(runner.Command.Env, "FABRIC_LOGGING_SPEC=orderer.consensus.smartbft=debug:grpc=debug")
			proc := ifrit.Invoke(runner)
			ordererProcesses[1] = proc
			Eventually(proc.Ready(), network.EventuallyTimeout).Should(BeClosed())
			Eventually(runner.Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 0, sequence 8"))
			...
			Eventually(runner.Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Node 2 was informed of a new view 2 channel=testchannel1"))
		...
		It("smartbft node addition and removal", func() {
			...
			waitForBlockReceptionByPeer(peer, network, "testchannel1", 10)
			...
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Synchronized to view 0 and sequence 12 with verification sequence 5 channel=testchannel1"))
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 1, sequence 13, and decisions 0 channel=testchannel1"))
			By("Invoking again")
			...
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Deciding on seq 14"))
			...
			updateSmartBFTMetadata(network, peer, network.Orderers[2], "testchannel1", func(md *smartbft.ConfigMetadata) {
				md.Consenters = md.Consenters[:4]
			})
			updateSmartBFTMetadata(network, peer, network.Orderers[1], "systemchannel", func(md *smartbft.ConfigMetadata) {
				md.Consenters = md.Consenters[:4]
			})
			...
			By("Make sure the peers get the config blocks, again")
			waitForBlockReceptionByPeer(peer, network, "testchannel1", 15)
			...
			updateSmartBFTMetadata(network, peer, network.Orderers[2], channel, func(md *smartbft.ConfigMetadata) {
				md.Consenters = append(md.Consenters, &smartbft.Consenter{
					MspId:       "OrdererMSP",
					ConsenterId: 5,
					Identity: protoutil.MarshalOrPanic(&msp.SerializedIdentity{
			...
			Eventually(runner.Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Deciding on seq 19 channel=testchannel1"))
		})
		...
		It("smartbft iterated addition and iterated removal", func() {
			...
			for i := 0; i < 6; i++ {
				...
				updateSmartBFTMetadata(network, peer, lastOrderer, channel, func(md *smartbft.ConfigMetadata) {
		...
		It("smartbft multiple nodes view change", func() {
			...
			By("Waiting for view change to occur")
			Eventually(ordererRunners[0].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("ViewChanged, the new view is 1"))
		...
		It("smartbft reconfiguration prevents blacklisting", func() {
			...
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Message from 1 channel=testchannel1"))
			Eventually(ordererRunners[3].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Message from 1 channel=testchannel1"))
			Eventually(ordererRunners[4].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Message from 1 channel=testchannel1"))
			...
			By("Waiting for view change to occur")
			Eventually(ordererRunners[1].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 1, sequence 11, and decisions 0 channel=testchannel1"))
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 1, sequence 11, and decisions 0 channel=testchannel1"))
			Eventually(ordererRunners[3].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 1, sequence 11, and decisions 0 channel=testchannel1"))
			Eventually(ordererRunners[4].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Starting view with number 1, sequence 11, and decisions 0 channel=testchannel1"))
			...
			Eventually(ordererRunners[1].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Skipping verifying prev commits due to verification sequence advancing from 4 to 5 channel=testchannel1"))
			Eventually(ordererRunners[2].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Skipping verifying prev commits due to verification sequence advancing from 4 to 5 channel=testchannel1"))
			Eventually(ordererRunners[3].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Skipping verifying prev commits due to verification sequence advancing from 4 to 5 channel=testchannel1"))
			Eventually(ordererRunners[4].Err(), network.EventuallyTimeout, time.Second).Should(gbytes.Say("Skipping verifying prev commits due to verification sequence advancing from 4 to 5 channel=testchannel1"))
		})
	})
})
...
// updateSmartBFTMetadata executes a config update that updates the smartBFT
// metadata according to the given function f
func updateSmartBFTMetadata(network *nwo.Network, peer *nwo.Peer, orderer *nwo.Orderer, channel string, f func(md *smartbft.ConfigMetadata)) {
