...
func TestIsConsenterOfChannel(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			expectedError: "failed to unmarshal payload from envelope: error unmarshaling Payload: proto: common.Payload: illegal tag 0 (wire type 1)",
			configBlock: &common.Block{
		...
		t.Run(testCase.name, func(t *testing.T) {
			cryptoProvider, err := sw.NewDefaultSecurityLevelWithKeystore(sw.NewDummyKeyStore())
			assert.NoError(t, err)
			cc := &ConsenterCertificate{
				ConsenterCertificate: testCase.certificate,
				CryptoProvider:       cryptoProvider,
			}
			err = cc.IsConsenterOfChannel(testCase.configBlock)
...
func TestRemoteNodesFromConfigBlock(t *testing.T) {
	...
	config, err := RemoteNodesFromConfigBlock(block, 1, flogging.MustGetLogger("test"), cryptoProvider)
	assert.NoError(t, err)
