...
func RemoteNodesFromConfigBlock(block *common.Block, selfID uint64, logger *flogging.FabricLogger, bccsp bccsp.BCCSP) (*nodeConfig, error) {
	env := &common.Envelope{}
...
func (rtc RuntimeConfig) configBlockCommitted(block *common.Block) (RuntimeConfig, error) {
	nodeConf, err := RemoteNodesFromConfigBlock(block, rtc.id, rtc.logger, rtc.CryptoProvider)
	if err != nil {
