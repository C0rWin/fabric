...
func (v *Verifier) VerifyConsenterSig(signature types.Signature, prop types.Proposal) ([]byte, error) {
	...
	sig.SignatureHeader = protoutil.MarshalOrPanic(&common.SignatureHeader{
		Nonce:   sig.Nonce,
		Creator: identity,
	})
	...
	return sig.AuxiliaryInput, v.ConsenterVerifier.EvaluateSignedData([]*protoutil.SignedData{signedData})
}
...
func (cv *consenterVerifier) EvaluateSignedData(signatureSet []*protoutil.SignedData) error {
